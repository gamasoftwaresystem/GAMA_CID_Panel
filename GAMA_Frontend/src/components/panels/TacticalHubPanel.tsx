...
export const TacticalHubPanel: React.FC<TacticalHubPanelProps> = ({
    ...
    return (
        <div className="glass-panel w-[850px] h-[450px] p-5 rounded-3xl hud-panel-enter shadow-[0_0_60px_rgba(0,0,0,0.9)] overflow-hidden pointer-events-auto border-t border-white/10">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        ...
                        <h2 className="text-[13px] font-black text-white tracking-[0.1em] uppercase">
                            {selectedDrone ? `Tactical HUB: ${selectedDrone.drone_id}` : 'Tactical HUB: Fleet Info'}
                        </h2>
                    </div>
                </div>
                            ...
                            <div className="grid grid-cols-2 gap-3 flex-1">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`border rounded-2xl p-4 flex flex-col items-center justify-center transition-all group ${activeCommand === cmd.label ? 'bg-white/10 border-white/30 scale-95 shadow-inner' : `bg-black/40 border-white/5 ${cmd.bg}`}`}>
                                        <cmd.icon className={`w-5 h-5 mb-3 transition-colors ${activeCommand === cmd.label ? 'text-white' : cmd.color}`} />
                                        <span className="text-[10px] font-black uppercase tracking-widest text-white/70 group-hover:text-white">{cmd.label}</span>
                                    </button>
                                ))}
                            </div>
                        </div>
